                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div className="field-group">
                    ...
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Defaults to today. Choose a past date to backdate this entry.
                    </p>
                  </div>
                </div>
                {/* Historical entry toggle — auto-computed from date, user-overridable.
                    See the entry_type_override comment near effectiveEntryType for
                    the "sticky by design" override semantics. */}
                <div className="flex items-start justify-between gap-3 rounded-md border border-border bg-muted/30 p-2.5">
                  <div className="min-w-0">
                    <Label htmlFor="historical-toggle" className="field-label cursor-pointer">
                      Backfilled / historical entry
                    </Label>
                    <p className="text-[10px] text-muted-foreground mt-0.5">
                      Automatically marked based on date — override if needed.
                    </p>
                  </div>
                  <Switch
                    id="historical-toggle"
                    checked={isMarkedHistorical}
                    onCheckedChange={(checked) => {
                      // If the new checked state matches the auto-computed value,
                      // clear the override so future date edits resume auto-computing.
                      // Otherwise persist the explicit user choice.
                      setForm((p) => ({
                        ...p,
                        entry_type_override:
                          checked === autoHistorical ? null : checked ? "historical" : "current",
                      }));
                    }}
                  />
                <div className="grid grid-cols-2 gap-2">
