...
export default function BylawsGenerator({ companyId, companyName, company }: Props) {
  ...
  const buildPdf = (aiDraftSections: Record<string, string> | null = null) => {
    const data: BylawsData = { company, directors, officers, shareholders, aiDraftSections };
    const doc = generateBylawsPDF(data);
    setPdfDoc(doc);
  ...
  const handleClientGenerate = () => {
    ...
    try {
      buildPdf(null);
      toast.success("Bylaws generated!");
    } catch (err: any) {
  ...
  const handleAiGenerate = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/generate-bylaws`,
        {
          method: "POST",
          headers: {
      ...
      toast.success("AI-drafted Bylaws generated!");
    } catch (err: any) {
      console.error(err);
      toast.error(err.message);
    } finally {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await supabase.from("document_registry").insert({
        ...
        title: `Bylaws — ${new Date().toLocaleDateString()}`,
        document_category: "corporate",
        document_type: "Bylaws",
        status: "final",
        file_name: fileName,
        file_url: signedData.signedUrl,
        statute_reference: "Wis. Stat. Ch. 180",
      });
    ...
    <div className="space-y-5 animate-fade-in">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between flex-wrap gap-3">
            <div className="flex items-center gap-2.5">
              ...
              <CardTitle className="text-base font-display">Bylaws</CardTitle>
            </div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Generate comprehensive corporate Bylaws under the Wisconsin Business Corporation Law. Choose standard generation or AI-assisted drafting customized to your company's details.
          </p>
        </CardHeader>
      ...
      {previewUrl && (
        <Card>
          ...
          <CardContent>
            <iframe src={previewUrl} className="w-full h-[600px] rounded border border-border" title="Bylaws Preview" />
          </CardContent>
        </Card>
      )}
    </div>
