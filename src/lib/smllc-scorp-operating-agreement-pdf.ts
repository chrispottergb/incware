...
export function generateSMScorpOperatingAgreementPDF(data: SMScorpOperatingAgreementData): jsPDF {
  ...
  y = addParagraph(doc, y,
    `If the Company issues membership certificates or statements of ownership, they shall contain the following legend: "THE MEMBERSHIP INTERESTS EVIDENCED HEREBY ARE SUBJECT TO TRANSFER RESTRICTIONS SET FORTH IN THE COMPANY'S OPERATING AGREEMENT INTENDED TO PRESERVE THE COMPANY'S S CORPORATION ELECTION UNDER IRC § 1362. TRANSFER TO ANY PERSON OR ENTITY THAT IS NOT ELIGIBLE TO OWN STOCK IN AN S CORPORATION UNDER IRC §1361 IS VOID."`
  );
