...
/**
 * Amendment mode for {@link analyzePreExistingLedger}.
 *
 * When a locked snapshot is being superseded, the ledger rows that snapshot
 * generated are EXPECTED to be there — the amendment RPC marks them
 * `corrected` inside its own transaction. Treating them as blocking activity
 * would make every amendment impossible. Any OTHER live row on or before the
 * as-of date still blocks, with the unchanged message.
 */
export interface AmendmentContext {
  /** `share_transactions.id` values generated by the snapshot being superseded. */
 ...
 *  - In amendment mode, rows generated by the snapshot being superseded are
 *    ignored too: the amendment RPC corrects them atomically as part of the
 *    same transaction.
 */
export function analyzePreExistingLedger(
  rows: PriorLedgerRow[],
  asOfDate: string,
  amendment?: AmendmentContext
): PriorLedgerAnalysis {
  ...
  const superseded = new Set(amendment?.supersededTransactionIds ?? []);
  const live = rows.filter(
    (r) => (r.status ?? "") !== "corrected" && !(r.id && superseded.has(String(r.id)))
  );
