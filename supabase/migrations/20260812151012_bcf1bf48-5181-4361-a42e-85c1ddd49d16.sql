...
BEGIN
  ...
  -- Certificate cancellation is conditional: uncertificated entities never get
  -- cancellation records for documents that do not exist.
  ...
  LOOP
    ...
    IF v_lot->>'status' = 'surrendered' THEN
      ...
      INSERT INTO ownership_snapshot_lots (
        ...
        NULL, nullif(btrim(coalesce(v_lot->>'notes','')), '')
      );
      CONTINUE;
    END IF;
